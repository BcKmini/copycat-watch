...
| **CLIP 의미적 유사도** (옵션) | phash+colorhash가 놓치는 의미적 유사도(각도·배경·리터칭)를 CLIP(ViT-B/32, ONNX) 임베딩으로 보강한다. 게이팅은 검증된 해시 기준을 유지하고 CLIP은 점수·정렬만 개선. `onnxruntime`이 있으면 활성화되고 없으면 자동 폴백하는 **옵션 모듈**이다. (현재 라이브 이미지는 신고서 LLM(llama.cpp)과의 프로세스 충돌을 피해 LLM을 우선해 CLIP 미포함으로 배포) |
...
### 어디를 탐색해서 결과를 내는가 (탐색 범위 & 지원 플랫폼)

**스캔 범위는 특정 사이트에 한정되지 않는다.** 1단계 Google Vision Web Detection이 **인터넷
전체**를 대상으로 내 이미지와 동일/유사한 이미지가 게시된 페이지를 찾고, 2단계에서 서버가 그
이미지를 직접 내려받아(또는 페이지를 방문해) 실측 검증한다. 즉 **웹에 공개돼 검색엔진에 인덱싱된
곳이면 어느 사이트든 탐지 대상**이며, 결과에는 발견된 게시물의 실제 URL이 그대로 들어간다.

그중 아래 **17개 주요 플랫폼은 자동으로 인식**해, 신고서·법적 대응 가이드에 그 플랫폼의 실제
신고 채널과 절차를 맞춰서 넣는다(목록에 없는 사이트는 일반 신고 절차로 안내).

| 구분 | 자동 인식·맞춤 안내되는 플랫폼 |
|---|---|
| 오픈마켓·쇼핑 | 쿠팡 · 네이버 스마트스토어 · 11번가 · 지마켓 · 옥션 · 위메프 · 티몬 · 알리익스프레스 |
| 중고거래 | 당근마켓 · 번개장터 |
| SNS·영상 | 인스타그램 · 페이스북 · 스레드 · X(트위터) · 틱톡 · 유튜브 · 핀터레스트 |

> 플랫폼마다 **신고 절차가 다르게** 안내된다 — 예: 쿠팡 → "판매자신고센터 접수, 처리 12일~12주",
> 당근마켓 → "앱 내 신고하기", 인스타그램 → "Meta 저작권 신고 양식", 알리익스프레스 → "IPP Center
> 권리자 등록". 여러 플랫폼에서 동시에 발견되면 각각의 건수와 절차를 모두 나열한다.
