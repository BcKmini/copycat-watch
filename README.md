...
K-AI 콘텐츠 공모전 Track B(솔루션 부문) 출품작. 소상공인이 겪는 "온라인 판매 사진
무단 도용" 문제를, 실시간 리버스 이미지 검색 + AI 문서 생성으로 해결한다.

> 🔗 배포 데모: 실행 방법은 [빠르게 실행해보기](#-빠르게-실행해보기) 참고
...
| 🕵️ **딥 페이지 검증** | 이미지 다운로드가 막힌 사이트는 페이지 HTML을 직접 방문해 `og:image`/본문 이미지를 추출해 대조한다 |
...
| 🏷️ **플랫폼 자동 감지** | 발견된 URL에서 쿠팡/네이버 스마트스토어/인스타그램 등 플랫폼을 자동 인식해 신고서 문구를 맞춘다 |
| 📦 **통합 신고서** | 여러 도용 사례를 체크박스로 선택해 하나로 묶은 통합 신고서 생성 |
| 🖼️ **이미지 비교** | 내 원본과 발견된 이미지를 나란히 놓고 비교 |
...
3. 스캔 결과 (유사도·검증여부·피해액·바로가기 링크)
        ...
        └──▶ 4b. 증거 리포트 다운로드 (원본 포함 HTML 문서)
```

Google Vision API 키가 없거나 호출이 실패하면, 자동으로 **내장 데모 데이터셋**(CC
라이선스 실사 이미지 40종 × 변형본 2장)에서 phash+colorhash로 매칭하는 폴백 모드로
...
| 테스트 | pytest(백엔드 28개), Playwright(프론트 시각 검증) |
...
│   ├── demo_data/               #   40개 상품 × 3장(원본+도용본 2장) CC 라이선스 이미지
│   ├── tests/                   #   pytest 28개 (매칭 로직 + API 엔드포인트)
...
├── EXPERIMENT.md                # 정확도 실험 전체 기록 (4 iteration)
└── README.md                    # 이 문서
```
...
- **`EXPERIMENT.md`**: CC 라이선스 실사 데이터셋으로 4단계에 걸쳐 진행한 정확도 실험
  ...
  투명하게 공개.
- **`backend/tests/`**: pytest 28개 — 정상 케이스뿐 아니라 손상된 파일, 초과 용량,
...
<sub>이 프로젝트는 K-AI 콘텐츠 공모전 출품을 위해 제작되었습니다. 신고서·증거 리포트는
자동 생성된 참고자료이며, 실제 법적 조치 전 전문가 검토를 권장합니다.</sub>
