...
한 상품에 대해 여러 곳에서 발견된 도용 사례를 하나로 묶어 아래 두 문서를 한국어로 작성해.

1. [통합 신고 사유서] - 발견된 모든 도용 사례를 표/목록 형태로 요약하고, 전체적으로
   조직적/반복적 도용임을 강조하는 사유서 (각 판매처별 유사도·정황 포함)
2. [통합 내용증명 초안] - 발견된 모든 판매처를 열거하며 일괄적으로 판매중지·손해배상을
   요구하는 내용증명 초안 (발신인/수신인 자리는 [ ]로 표시)
...
def generate_batch_report(req: BatchReportRequest):
    ...
    match_lines = "\n".join(
        ...
        + (f", 예상피해액: {m.estimated_damage:,}원" if m.estimated_damage else "")
        + ")"
    ...
    if not api_key:
        ...
        return {
            "report": (
                "---문서1---\n"
                f"[통합 신고 사유서 - 임시 템플릿]\n"
                f"본인이 판매 중인 '{req.product_name}' 상품 이미지가 아래 {len(req.matches)}곳에서 "
                f"무단으로 사용되고 있음을 확인했습니다:\n{listing}\n\n"
                f"총 예상 피해액은 {total_damage:,}원으로 산정되며, 각 게시물의 판매 중지 및 "
                "이미지 삭제 조치를 일괄 요청합니다.\n\n"
                "---문서2---\n"
                "[통합 내용증명 초안 - 임시 템플릿]\n"
                "발신인: [본인 성명/상호]\n수신인: [각 상호/성명]\n\n"
                f"귀하들이 판매 중인 상품의 이미지가 본인이 판매 중인 '{req.product_name}' 상품 사진과 "
                "동일하거나 매우 유사함을 다수 확인하였습니다. 이는 저작권 침해에 해당하며, "
                "본 통지 수령 후 7일 이내 해당 상품의 판매를 중단하고 이미지를 삭제할 것을 요청합니다.\n\n"
                "[ANTHROPIC_API_KEY 미설정 - console.anthropic.com에서 키 발급 후 .env에 추가하면 AI가 상황별로 맞춤 작성합니다]"
            )
        }
